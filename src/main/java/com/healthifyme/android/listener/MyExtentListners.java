...
package com.healthifyme.android.listener;
...
import com.healthifyme.android.library.ExcelLibrary;
...
public class MyExtentListners implements ITestListener {
	...
	public void onStart(ITestContext context) {
		...
		if (String.valueOf(deviceCount.size()).equalsIgnoreCase("1")) {
			for (int i = 1; i <= rowCount; i++) {
				...
				if (ExcelLibrary.getExcelData(GenericLib.sConfigPath, "config", i, runStatus).equalsIgnoreCase("Yes")) {
					...
					extent.setSystemInfo("Application Name", "HealthifyMe");
					...
					extent.setSystemInfo("Android Version",
		...
		htmlReporter.config().setDocumentTitle("HealthifyMe Automation");
		htmlReporter.config().setReportName("HealthifyMe Automation Test Report");
